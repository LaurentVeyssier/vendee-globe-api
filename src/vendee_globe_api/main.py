...
try:
    ...
    df_infos = pd.read_parquet(
        c.df_infos_path
    )  # Importing the skippers and boats data into a DataFrame
    df_race = pd.read_parquet(
        c.df_race_path
    )  # Importing the race data into a DataFrame
...
def get_race():
    ...
    try:
        logger.info(
            f"Request received on /race (current_batch_index={app.state.current_batch_index})"
        )
        ...
        if (
            app.state.current_batch_index >= total_batches
        ):  # Check if the current batch index exceeds the total number of batches
            logger.warning("Attempted to access data beyond available batches.")
            return {"message": "No more available data."}

        batch_values = unique_batches[
            : app.state.current_batch_index + 1
        ]  # Get batch values up to the current index
        batch_data = df_race[
            df_race["batch"].isin(batch_values)
        ]  # Filter the DataFrame to include only the selected batches
...
async def batch_scheduler():
    ...
    while True:
        ...
        try:
            if (
                app.state.current_batch_index <= total_batches - 1
            ):  # Check if the current batch index is less than the total number of batches
...
async def startup_event():  # async function to run on startup
    ...
    asyncio.create_task(
        batch_scheduler()
    )  # Start the batch scheduler in the background
